...
# ==========================================
# Ép FaceNet/ONNXRuntime sử dụng CUDA để tăng tốc bằng NVIDIA GPU (RTX 4050)
...
# Cấu hình log cho môi trường Windows (Đã xóa cấu hình Linux gây lỗi)
os.environ["QT_LOGGING_RULES"] = "*.debug=false;qt.qpa.*=false;qt.text.font.*=false"
os.environ["OPENCV_LOG_LEVEL"] = "FATAL"

# Diệt gọn log của MediaPipe, TensorFlow và ONNX
...
def init_system_resources():
    ...
    def heavy_vision_thread():
        print("[INFO] Đang khởi động luồng AI Thị giác (Nặng) bằng CUDA trên RTX 4050...")
        try:
            # LƯU Ý: Trỏ file model_path về file YOLO .pt gốc thay vì thư mục OpenVINO
            # device="0" để kích hoạt GPU NVIDIA đầu tiên
            yolo_model = ObjectDetector(model_path="weights/best (1).pt", device="0")
        ...
        vision_ready.set()
        print("[INFO] ✅ AI Thị giác (Nặng) đã nạp xong trên NVIDIA GPU!")
